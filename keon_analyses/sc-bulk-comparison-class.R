...
load("rawdata/MRI_Values.RData")

mri = mriT %>% t() %>% as.data.frame() %>% rownames_to_column(var = "UT.code")%>%
  inner_join(within_meta, by = "UT.code")
mri = mri[,c(2,5)]
names(mri)[1] = "MRI.thickness"
...
marker_data = read.csv("https://raw.githubusercontent.com/sonnyc247/MarkerSelection/master/Data/Outputs/CSVs_and_Tables/Markers/All_hodge_regions/new_ALLReg_results_class_ITexpand_lfct15_minpct25_dup.csv")[,-1]
...
(cell_types = marker_data$class %>% unique())
# Organize markers into a list 
marker_list = lapply(cell_types, function(cell_type){
  return(marker_data %>% filter(class == cell_type) %>% pull(gene) %>% unlist())
})
...
pdf("keon_analyses/output/correlation-heatmap-class.pdf",width=8,height=6)
